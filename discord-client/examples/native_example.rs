...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bot_token =
        env::var("DISCORD_BOT_TOKEN").expect("DISCORD_BOT_TOKEN environment variable must be set");
    ...
    let edited = client
        .edit_message(&channel_id, &response_id, &edit)
        .await?;
    ...
    let (file_data, filename, description) = match env::var("DISCORD_IMAGE_PATH") {
        Ok(path_str) => {
            ...
            (
                bytes,
                fname,
                Some("Uploaded from DISCORD_IMAGE_PATH".to_string()),
            )
        }
        Err(_) => {
    ...
    let attachment_response = client
        .send_message(&channel_id, &attachment_message)
        .await?;
    println!(
        "Attachment message sent! ID: {} (attachments: {})",
        attachment_response.id,
        attachment_response.attachments.len()
    );

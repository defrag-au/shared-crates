use discord_client::{DiscordMessage, DiscordAttachment, DiscordEmbed, NativeDiscordClient, DiscordClient};
use std::env;
use tokio;
...
async fn test_native_message_with_embed() {
    ...
    let embed = DiscordEmbed {
        ...
        description: Some("This is a test embed from discord-client".to_string()),
        color: Some(0x00ff00),
        thumbnail: None,
        image: None,
        fields: vec![],
    ...
    let result = client.send_message(&channel_id, &message).await;
    assert!(result.is_ok(), "Failed to send message with embed: {:?}", result);
}
...
async fn test_native_message_with_attachment() {
    ...
    let result = client.send_message(&channel_id, &message).await;
    assert!(result.is_ok(), "Failed to send message with attachment: {:?}", result);
}
...
fn test_content_type_detection() {
    assert_eq!(NativeDiscordClient::get_content_type("test.png"), "image/png");
    assert_eq!(NativeDiscordClient::get_content_type("test.jpg"), "image/jpeg");
    assert_eq!(NativeDiscordClient::get_content_type("test.jpeg"), "image/jpeg");
    assert_eq!(NativeDiscordClient::get_content_type("test.gif"), "image/gif");
    assert_eq!(NativeDiscordClient::get_content_type("test.webp"), "image/webp");
    assert_eq!(NativeDiscordClient::get_content_type("test.unknown"), "application/octet-stream");
}
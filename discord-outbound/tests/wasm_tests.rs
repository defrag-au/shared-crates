...
mod wasm_tests {
    ...
    use discord_outbound::{AttachmentInput, DiscordClient, DiscordMessage, WasmDiscordClient};
    ...
    async fn test_wasm_json_message() {
        ...
        let bot_token = match search.contains("bot_token=") {
            true => {
                let url_encoded = search
                    ...
                    .next()
                    .unwrap();
                js_sys::decode_uri_component(url_encoded)
                    .unwrap()
                    .as_string()
                    .unwrap()
            }
            false => {
                web_sys::console::log_1(
                    &"Skipping test: bot_token not provided in query params".into(),
                );
                return;
            }
        };
        ...
        let channel_id = match search.contains("channel_id=") {
            ...
            false => {
                web_sys::console::log_1(
                    &"Skipping test: channel_id not provided in query params".into(),
                );
                return;
            }
        };
        ...
        let message = DiscordMessage {
            content: Some("Test message from discord-outbound WASM".to_string()),
            embeds: None,
            attachments: None,
        };
    ...
    async fn test_wasm_message_with_embed() {
        ...
        let embed = TwEmbedBuilder::new()
            ...
            .description("This is a test embed from discord-outbound WASM")
            .color(0x0099ff)
            .build();
        ...
        assert!(
            result.is_ok(),
            "Failed to send WASM message with embed: {:?}",
            result
        );
    }
    ...
    async fn test_wasm_message_with_attachment() {
        ...
        assert!(
            result.is_ok(),
            "Failed to send WASM message with attachment: {:?}",
            result
        );
    }
    ...
    fn test_wasm_content_type_detection() {
        ...
        assert_eq!(
            WasmDiscordClient::get_content_type("test.jpg"),
            "image/jpeg"
        );
        assert_eq!(
            WasmDiscordClient::get_content_type("test.jpeg"),
            "image/jpeg"
        );
        assert_eq!(WasmDiscordClient::get_content_type("test.gif"), "image/gif");
        assert_eq!(
            WasmDiscordClient::get_content_type("test.webp"),
            "image/webp"
        );
        assert_eq!(
            WasmDiscordClient::get_content_type("test.unknown"),

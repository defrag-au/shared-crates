...
//!
//! Ported from the egui storybook's fixture of the same name, so both renderers
//! are reviewed against the same chain. Blocks are drawn on the chain's real
//! tempo (each one-second slot a 5% chance) from a fixed seed, so every run
//! looks the same and screenshots are stable. The events go through a real
//! `chain_heartbeat::Heartbeat`, exactly as a host would apply them, so the
//! widgets are shown folding the data they will fold in production rather than a
//! hand-built snapshot.
//!
//! The clock is the story's own: it advances with macroquad's frame time
//! multiplied by a speed the reader controls, and starts an hour into the chain
//! so the train has history from the first frame.
...
impl ChainSim {
    ...
    pub fn advance(&mut self) {
        // Clamp a frame gap (a window that was not being drawn) so the story
        // does not jump.
        let delta = (get_frame_time() as f64).clamp(0.0, 0.5);
...
/// A chain from `after_slot` (exclusive) to `until_slot`, starting at
/// `first_height`.

//! Storybook demo for the `UtxoShelf` widget — UTxO health shelving.
//!
//! Lived at `stories/utxo_map.rs` behind a `Story::UtxoMap` variant until the
//! theming pass went looking for a UtxoMap story to review a colour change in
//! and found this instead: the file name and the variant both said "map", the
//! label said "Shelf", and the code rendered `ShelfConfig`. Only the label was
//! right. `utxo_map` is now the terrain map it always claimed to be.
...
pub struct UtxoShelfStoryState {
...
impl Default for UtxoShelfStoryState {
    fn default() -> Self {
...
pub fn show(
    ...
    if let Some(ref data) = state.data {
        ...
        ui.add_space(40.0);
        ui.label(
            egui::RichText::new("Connect a wallet and fetch UTxOs to visualize").color(muted(ui)),
        );
    }
}

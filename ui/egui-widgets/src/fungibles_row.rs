...
pub struct FungiblesRowConfig {
    ...
    pub ticker_size: f32,
    pub ticker_text_color: Option<Color32>,
    pub ticker_bg: Option<Color32>,
    pub ticker_corner_radius: u8,
    pub ticker_padding_x: f32,
    pub ticker_padding_y: f32,
    pub qty_size: f32,
    pub qty_color: Option<Color32>,
    pub value_size: f32,
        ...
        ui.horizontal(|ui| {
            ...
            ui.label(
                RichText::new(self.display_name)
                    .size(cfg.name_size)
                    .color(cfg.name_color.unwrap_or(ui.tokens().color.text_primary))
                    .strong(),
            );
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                ui.label(
                    RichText::new(self.quantity_text)
                        .size(cfg.qty_size)
                        .color(cfg.qty_color.unwrap_or(ui.tokens().color.text_primary))
                        .monospace(),
                );
            });
        });
...
fn draw_chip(ui: &mut Ui, label: &str, cfg: &FungiblesRowConfig) {
    let t = ui.tokens();
    let ticker_text = cfg.ticker_text_color.unwrap_or(t.color.text_secondary);
    let ticker_bg = cfg.ticker_bg.unwrap_or(t.color.bg_highlight);

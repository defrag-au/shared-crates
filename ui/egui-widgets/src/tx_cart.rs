...
pub fn show(
    ...
    ui.horizontal(|ui| {
        ...
        if !state.items.is_empty() {
            ui.label(
                RichText::new(format!("({})", state.items.len()))
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            if !state.items.is_empty()
                && matches!(state.phase, TxCartPhase::Editing | TxCartPhase::Done)
            {
                if ui
                    .add(
                        egui::Button::new(
        ...
        .show(ui, |ui| {
            ...
            for item in &state.items {
                ui.horizontal(|ui| {
                    // Status indicator
                    ui.label(
                        RichText::new(match &item.status {
                            TxCartItemStatus::Pending => "\u{25CB}",       // empty circle
                            TxCartItemStatus::Building => "\u{25D4}",      // half circle
                            TxCartItemStatus::Signing => "\u{25D4}",       // half circle
                            TxCartItemStatus::Submitted { .. } => "\u{25CF}", // filled circle
                            TxCartItemStatus::Error { .. } => "\u{2716}",  // X mark
                        })
                    ...
                    ui.vertical(|ui| {
                        ...
                        ui.label(
                            RichText::new(format!("{} | {}", item.detail, item.provider))
                                .color(theme::TEXT_MUTED)
                                .size(9.0),
                        );
                    });
    ...
    match &state.phase {
        TxCartPhase::Editing => {
            if state.pending_count() > 0 {
                if ui
                    .add(
                        egui::Button::new(
                            RichText::new(format!("Execute {} item(s)", state.pending_count()))
        ...
        TxCartPhase::Preview => {
            ...
            for (i, planned) in state.planned_txs.iter().enumerate() {
                ui.horizontal(|ui| {
                    ui.label(
                        RichText::new(format!("TX {}: {}", i + 1, planned.summary))
                            .color(theme::TEXT_PRIMARY)
                            .size(10.0),
                    );
                    ui.label(
        ...
        TxCartPhase::Executing { total, completed } => {
            ui.horizontal(|ui| {
                ui.spinner();
                ui.label(
                    RichText::new(format!("Executing {completed}/{total}..."))
                        .color(theme::ACCENT_CYAN)
                        .size(11.0),
                );
            });

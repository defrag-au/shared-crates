...
//! clock, no socket, no runtime.

use std::collections::HashMap;
use std::time::Duration;

use chain_heartbeat::{
    BlockPoller, BlockTxs, ChainEvent, HeartbeatFrame, PollDecision, PollTiming, TxInBlock,
};
...
/// A transaction this frontend submitted.
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum ChainLiveEvent {
    /// A tracked transaction is in a block. Once per transaction, and only for
    /// what the crate found itself — in a block the feed delivered, or from a
    /// polled status source — never echoed back from `ChainLive::landed`.
    Landed(TxLanding),
...
impl Tracker {
    pub(crate) fn new(timing: PollTiming, asking: Asking) -> Self {
        Self {
            txs: Vec::new(),
    ...
    pub(crate) fn landed(&mut self, tx_hash: &str, block_height: Option<u64>, now_ms: u64) -> bool {
    ...
    pub(crate) fn dropped(&mut self, tx_hash: &str) {
    ...
    pub(crate) fn forget(&mut self, tx_hash: &str) {
        self.poller.resolve(&tx_hash.to_string());
        self.block_lookups.remove(tx_hash);
    ...
    pub(crate) fn observe_block(&mut self, txs: &BlockTxs, now_ms: u64) -> Vec<ChainLiveEvent> {
            ...
            match found {
                TxInBlock::Valid => {
                    if self.landed(&tx_hash, Some(txs.height), now_ms) {
                        seen.push(ChainLiveEvent::Landed(landing));
                    }
                }
                TxInBlock::FailedValidation => {
                    if self.failed_in_block(&tx_hash, txs.height) {
                        seen.push(ChainLiveEvent::FailedInBlock(landing));
                    }
                }
            }
        }
    ...
    pub(crate) fn rolled_back(&mut self, tip_height: Option<u64>, now_ms: u64) {
    ...
    pub(crate) fn next_poll(&mut self, now_ms: u64) -> PollStep {
    ...
    pub(crate) fn check_finished(
        ...
        let landings = result.unwrap_or_else(|e| {
            log::warn!("chain-live: status check failed: {e}");
            Vec::new()
        });
        match self.asking {
                ...
                for hash in asked {
                    ...
                    *lookups += 1;
                    if *lookups >= MAX_BLOCK_LOOKUPS {
                        log::debug!("chain-live: no block found for {hash}; leaving it off");
    ...
    pub(crate) fn riders(&self) -> Vec<TrainRider> {
        self.txs
            .iter()
            .filter_map(|tracked| {
                let state = match tracked.progress {
                    ...
                    // Still on the train as a status row while its block is
                    // found: dropping it there is what made the status vanish
                    // between "waiting" and "in a block".
...
mod tests {
    use super::*;
    use chain_heartbeat::{BlockBeat, ChainEvent, SyncState};
    ...
    fn a_landing_is_news_once_and_rides_its_block() {
        ...
        assert_eq!(
            tracker.riders(),
            vec![
                TrainRider::new("Your swap", RiderState::InBlock { height: 100 }),
                TrainRider::new("Your claim", RiderState::Waiting),
            ]
        );
        assert!(
            tracker
    ...
    fn an_unnamed_block_is_off_the_train_but_a_known_one_is_kept() {
        ...
        // yet keeps its status row as `Landed`, rather than leaving the train
        // until the block is found.
        assert_eq!(
            tracker.riders(),
            vec![
                TrainRider::new("Your swap", RiderState::InBlock { height: 100 }),
                TrainRider::new("Your buy", RiderState::Landed),
            ]
        );
    }
    ...
    fn a_reported_landing_without_a_block_is_looked_up_and_placed() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                TrainRider::new("Your offer", RiderState::Waiting),
            ]
        );
    ...
    fn a_tx_seen_in_a_block_lands_there_at_once_in_any_mode() {
        for asking in [Asking::Landings, Asking::BlocksOnly, Asking::Nothing] {
            ...
            assert_eq!(
                seen,
                vec![ChainLiveEvent::Landed(TxLanding {
                    tx_hash: HASH_A.into(),
                    block_height: Some(100),
                })],
                "{asking:?}"
            ...
            assert_eq!(
                ...
                vec![
                    TrainRider::new("Your swap", RiderState::InBlock { height: 100 }),
                    TrainRider::new("Your buy", RiderState::Waiting),
                ],
                "{asking:?}"
            );
    ...
    fn a_failed_tx_is_failed_and_no_later_report_turns_it_green() {
        ...
        assert_eq!(
            seen,
            vec![ChainLiveEvent::FailedInBlock(TxLanding {
                tx_hash: HASH_A.into(),
                block_height: Some(100),
            })]
        );
    ...
    fn a_resync_catches_a_landing_made_while_away() {
        ...
        let frame = HeartbeatFrame::Resync {
            checkpoint: chain_heartbeat::Checkpoint {
                network: chain_heartbeat::Network::Mainnet,
                beats: Vec::new(),
            },
            feed: chain_heartbeat::FeedHealth::NotStarted,
